...
4. 슬라이드에 로고를 넣으려면 상단 `Logo` 버튼에서 현재 페이지만 넣을지, 모든 페이지에 넣을지 선택한 뒤 이미지 파일을 선택합니다.
...
슬라이드 화면 상단의 `Logo` 버튼을 누르면 로고 이미지 파일을 선택할 수 있습니다. 메뉴에서 `Current page`를 고르면 현재 보고 있는 페이지만, `All pages`를 고르면 이름 있는 모든 페이지 컴포넌트에 같은 로고가 들어갑니다.
